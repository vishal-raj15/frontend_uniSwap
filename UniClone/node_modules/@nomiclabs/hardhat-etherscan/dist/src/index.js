...
const verifySubtask = async ({ address, constructorArguments, contract: contractFQN, libraries }, { config, network, run }) => {
    ...
    if (etherscan.apiKey === undefined || etherscan.apiKey.trim() === "") {
        throw new plugins_1.NomicLabsHardhatPluginError(constants_1.pluginName, `Please provide an Etherscan API token via hardhat config.
...
If your constructor has no arguments pass an empty array. E.g:
  ...
  await run("${constants_1.TASK_VERIFY_VERIFY}", {
    ...
    const etherscanAPIEndpoints = await run(constants_1.TASK_VERIFY_GET_ETHERSCAN_ENDPOINT);
    const deployedBytecodeHex = await prober_1.retrieveContractBytecode(address, network.provider, network.name);
...
Possible causes are:
  ...
  - The selected network (${network.name}) is wrong.`;
    ...
    const verificationStatus = await attemptVerification(etherscanAPIEndpoints, contractInformation, address, etherscan.apiKey, contractInformation.compilerInput, solcFullVersion, deployArgumentsEncoded);
...
config_1.subtask(constants_1.TASK_VERIFY_GET_CONSTRUCTOR_ARGUMENTS)
...
config_1.subtask(constants_1.TASK_VERIFY_GET_LIBRARIES)
...
async function attemptVerification(etherscanAPIEndpoints, contractInformation, contractAddress, etherscanAPIKey, compilerInput, solcFullVersion, deployArgumentsEncoded) {
    ...
    const response = await EtherscanService_1.verifyContract(etherscanAPIEndpoints.apiURL, request);
...
`);
    const pollRequest = EtherscanVerifyContractRequest_1.toCheckStatusRequest({
        apiKey: etherscanAPIKey,
...
async function inferContract(artifacts, network, matchingCompilerVersions, deployedBytecode) {
    const contractMatches = await bytecode_1.lookupMatchingBytecode(artifacts, matchingCompilerVersions, deployedBytecode);
...
config_1.subtask(constants_1.TASK_VERIFY_GET_COMPILER_VERSIONS).setAction(async (_, { config }) => {
...
config_1.subtask(constants_1.TASK_VERIFY_GET_ETHERSCAN_ENDPOINT).setAction(async (_, { network }) => prober_1.getEtherscanEndpoints(network.provider, network.name));
config_1.subtask(constants_1.TASK_VERIFY_GET_CONTRACT_INFORMATION)
    ...
    if (contractFQN !== undefined) {
        ...
        if (!contract_names_1.isFullyQualifiedName(contractFQN)) {
...
Possible causes are:
  ...
  - The selected network (${network.name}) is wrong.`);
        ...
        const { sourceName, contractName } = contract_names_1.parseFullyQualifiedName(contractFQN);
        contractInformation = await bytecode_1.extractMatchingContractInformation(sourceName, contractName, buildInfo, deployedBytecode);
...
Possible causes are:
  ...
  - The selected network (${network.name}) is wrong.`);
    ...
    const { libraryLinks, undetectableLibraries } = await libraries_1.getLibraryLinks(contractInformation, libraries);
    return Object.assign(Object.assign({}, contractInformation), { libraryLinks,
        undetectableLibraries });
});
config_1.subtask(constants_1.TASK_VERIFY_VERIFY_MINIMUM_BUILD)
...
config_1.subtask(constants_1.TASK_VERIFY_GET_MINIMUM_BUILD)
    .addParam("sourceName", undefined, undefined, config_1.types.string)
    .setAction(getMinimumBuild);
config_1.task(constants_1.TASK_VERIFY, "Verifies contract on Etherscan")
    ...
    .setAction(verify);
config_1.subtask(constants_1.TASK_VERIFY_VERIFY)
    .addParam("address", undefined, undefined, config_1.types.string)
